...
import com.ericsson.bos.so.security.mtls.config.WebClientReloadConfig;
...
/**
 * This class reloads the Web Client configurations when ever there is a change in
 * the certificate. Clients should not call this class.
 */
@Slf4j
@Component
@ConditionalOnProperty(value = "security.tls.enabled", havingValue = "true")
public class WebClientReload implements MtlsConfigurationReloader {

    @Autowired
    private SslContextUtil sslContextUtil;

    @Autowired
    @Qualifier(WebClientReloadConfig.SIP_TLS_QUALIFIER)
